...
Based on the line of best fit and what can be seen on the graph here, I would say that there is a slightly positive relationship between the FARMS percentage and the math test scores. A lot of the points earlier on the graph seem to not have a pattern to them, but with the line of best fit we are able to observe the positive correlation. 
...
This model is probably not worth using. Being that the p-value is greater than 0.05, it is showing us that the data is not statistically significant. Therefore, it is probably not a worthwhile representation of the information and there is no profound relationship between the two.
...
There are some pretty large differences between actual and predicted math scores that are shown in the data. The schools that saw the biggest differences were Rupert, Whitehall, Barth, Franklin, and Lincoln.
...
After hearing this, it would definitely influence the pitch and direction of my story. If this was true, it would absolutely be compelling way to start off the story. I would then tie it to other statistics that we have witnessed in this chart. For example, schools with fewer students on free lunch programs have higher test scores and higher predicted scores, or something along those lines. 
