...
When looking at the scatterplots, we can see that there is a positive correlation between voter turnout and most of the independent variables, but there is a negative correlation for population and percent unemployed. None appear to be linear, but percent bachelor's or higher is the closest relationship to linear. When viewing the correlation matrix, we see that there is zero correlation between median_age and pct_bachelors_higher. The highest correlation, as shown by this graph, is population and pct_white.
...
I would say that based on what I have observed, this is a useful model. The r-squared value of 0.854 indicates that 85% of the variation in turnout can be explained by the model. The model indicates that the numbers provided are statistically significant, which can be supported by the F-statistic. Overall, I would say this model also does a good job of predicting turnout as well. 
...
When comparing the models, it appears that they are all pretty significant, but model 4 is probably going to be the best, as it has the highest adjusted r-squared value. 
...
# Model 4
model4 <- lm(turnout ~ income + pct_bachelors_higher + pct_unemployed + median_age + pct_white, data = md_turnout)
summary(model4)
...
Similarly to how I pitched a story for a multiple regression model previously, I would cover and write about the the event by using simpler terms to describe the relationships between the numbers. It is important to be able to write a story that the average reader is able to comprehend. I would discuss realtionships between numbers and how certain variables affect voter turnout, but in the simplest form. 
